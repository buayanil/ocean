...
import { deleteDatabaseStart, getDatabaseStart } from '../../redux/slices/databaseSlice';
...
const DatabaseDetailView: React.FC<DatabaseDetailViewProps> = () => {
    ...
    const renderTabContent = (): React.ReactNode => {
        const database = databases.find(database => database.id === Number.parseInt(id));
        if (selectedId === 1) {
    ...
    const getHostFor = (database: DatabaseProperties): HostProperties | undefined => {
        if (database.engine === 'P') {
            ...
            return { hostname: REACT_APP_MONGODB_HOSTNAME || "", port: Number.parseInt(REACT_APP_MONGODB_PORT || "27017") }
        }
    }
        ...
        <AppLayout selectedNavigation={DatabasesNavigation.name}>
            <div className="flex space-x-3 mb-4">
                ...
                <div className="min-w-0 flex-1">
                    {database ?
                        <div className="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">{database.name}</div> :
            ...
            {/*Modal area*/}
            <DeleteModal open={openModal} modalContent={deleteModalContent} onSubmit={onSubmit} onClose={() => setOpenModal(false)} />
        </AppLayout>
    );
}
