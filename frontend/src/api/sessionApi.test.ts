...
describe('SessionApi', () => {
    ...
    it('throws an error when token refresh fails', async () => {
        mockedAxiosInstance.post.mockRejectedValueOnce(new Error('Token refresh failed')); // Simulate failure

        await expect(
            SessionApi.refreshToken({ refreshToken: 'invalidRefreshToken' })
        ).rejects.toThrow('Token refresh failed'); // Expect an error to be thrown
    });
    ...
    it('uses the refreshed token for subsequent requests', async () => {
        ...
        const nextResponse = await axiosInstance.post('/some-protected-endpoint', {}, {
            headers: { Authorization: `Bearer ${result.accessToken}` },
        });

        expect(nextResponse.data).toEqual({ success: true }); // Verify the subsequent request succeeds
        expect(mockedAxiosInstance.post).toHaveBeenCalledWith('/some-protected-endpoint', {}, {
