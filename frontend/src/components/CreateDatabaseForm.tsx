...
const CreateDatabaseForm: React.FC<CreateDatabaseFormProps> = ({ processing, onSubmit }) => {
    const createDatabaseSchema = yup.object().shape({
        name: yup.string()
            ...
            .test('unique_test', 'Name is already registered', (value, ctx) => validateDatabaseValues(value, ctx)),
        engine: yup.string().required('Engine is required'),
    })

    const validateDatabaseValues = async (name: string | undefined, context: yup.TestContext<Record<string, any>>): Promise<boolean> => {
                ...
                {({ errors, touched, values, setFieldValue, isValidating, isValid }) => (
                    <Form className="space-y-6">
                        <>
                            ...
                            <div className="text-sm font-light">Names must be lowercase and start with a letter. They can be between 4 and 32 characters long and may contain dashes.</div>
                            <div className="mt-3">
