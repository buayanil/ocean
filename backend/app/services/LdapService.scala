...
class LdapService @Inject() (config: Configuration) {
  ...
  def authenticate(username: String, password: String): Either[List[ErrorMessage], User] = {
    ...
    ldapConnectionConfig match {
      case Right(config) =>
        fetchLdapRole(config, username) match {
          case Right(entry) =>
            getUserForEntry(entry, username) match {
  ...
  def getUserForEntry(entry: Entry, username: String): Either[ErrorMessage, User] =
    try {
      ...
      case e: LdapInvalidAttributeValueException =>
        Left(
          ErrorMessage(
            ErrorMessage.CODE_LDAP_ENTRY_MISSING,
            ErrorMessage.MESSAGE_LDAP_ENTRY_MISSING,
  ...
  def fetchLdapRole(ldapConnectionConfig: LdapConnectionConfig, username: String): Either[ErrorMessage, Entry] = {
    ...
    try {
      ...
      case e: InvalidConnectionException =>
        return Left(
          ErrorMessage(
            ErrorMessage.CODE_LDAP_AUTHENTICATION_FAILED,
            ErrorMessage.MESSAGE_LDAP_AUTHENTICATION_FAILED,
            developerMessage = e.getMessage
          )
        )
    ...
    try {
      ...
      case e: CursorException =>
        return Left(
          ErrorMessage(
            ErrorMessage.CODE_LDAP_CURSOR_FAILED,
            ErrorMessage.MESSAGE_LDAP_AUTHENTICATION_FAILED,
            developerMessage = e.getMessage
          )
        )
    ...
    try ldapConnection.close()
    catch {
      case e: LdapException =>
        return Left(
          ErrorMessage(
            ErrorMessage.CODE_LDAP_CLOSING_FAILED,
            ErrorMessage.MESSAGE_LDAP_AUTHENTICATION_FAILED,
            developerMessage = e.getMessage
          )
        )
  ...
  def getLdapConnectionConfig(username: String, password: String): Either[List[ErrorMessage], LdapConnectionConfig] = {
    ...
    ldapHost match {
      ...
      case _ =>
        errors += ErrorMessage(
          ErrorMessage.CODE_LDAP_CONNECTION_CONFIG,
          ErrorMessage.MESSAGE_LDAP_CONNECTION_CONFIG,
          developerMessage = "Could not load ldap host."
        )
    }
    ...
    ldapPort match {
      ...
      case _ =>
        errors += ErrorMessage(
          ErrorMessage.CODE_LDAP_CONNECTION_CONFIG,
          ErrorMessage.MESSAGE_LDAP_CONNECTION_CONFIG,
          developerMessage = "Could not load ldap port."
        )
    }
    ...
    ldapStartTls match {
      ...
      case _ =>
        errors += ErrorMessage(
          ErrorMessage.CODE_LDAP_CONNECTION_CONFIG,
          ErrorMessage.MESSAGE_LDAP_CONNECTION_CONFIG,
          developerMessage = "Could not load ldap useTls."
        )
    }
    ...
    ldapUseSsl match {
      ...
      case _ =>
        errors += ErrorMessage(
          ErrorMessage.CODE_LDAP_CONNECTION_CONFIG,
          ErrorMessage.MESSAGE_LDAP_CONNECTION_CONFIG,
          developerMessage = "Could not load ldap useSsl."
        )
    }
        ...
        ldapName match {
          ...
          case _ =>
            errors += ErrorMessage(
              ErrorMessage.CODE_LDAP_CONNECTION_CONFIG,
              ErrorMessage.MESSAGE_LDAP_CONNECTION_CONFIG,
              developerMessage = "Could not load ldap name."
            )
        }
      case _ =>
        errors += ErrorMessage(
          ErrorMessage.CODE_LDAP_CONNECTION_CONFIG,
          ErrorMessage.MESSAGE_LDAP_CONNECTION_CONFIG,
          developerMessage = "Could not load ldap userRoot."
        )
    }
